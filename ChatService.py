...
def handle_neither(state: AgentState) -> dict:
    ...
    return {
        "final_response": "Sorry, I can only help with medical questions or storing your health data. If you have a medical question or want to update your health profile, just let me know!"}
...
def judge_advice(state: AgentState) -> dict:
    ...
    # 1.Strict System Prompt for the Judge (includes hallucination grounding)
    judge_template = """You are a medical safety auditor (Judge).
        ...
        4. The advice MUST be grounded in the provided medical knowledge. If the assistant makes specific factual medical claims that are NOT supported by the knowledge base below, mark as rejected and explain which claims lack grounding.

        RELEVANT MEDICAL KNOWLEDGE (used to generate the advice):
        {medical_knowledge}

        Assistant's Advice to evaluate:
        "{advice}"

        Evaluate if this advice strictly follows ALL the rules above, paying special attention to rule 4 (factual grounding).
    ...
    formatted_prompt = prompt.format_messages(
        advice=state["advice"],
        medical_knowledge=state.get("medical_knowledge", "No medical knowledge available")
    )
...
def fallback_response(state: AgentState) -> dict:
    ...
    return {
        "final_response": "Sorry, but I'm not able to help you with that. If you're worried you should go see a doctor."}


def store_conversation_summary(state: AgentState) -> dict:
    """Save a summary of this conversation turn for long-term memory"""
    print("\n💾 [MEMORY] Saving conversation summary to user profile...")
    userRepo.save_conversation_summary(
...
def build_graph() -> CompiledStateGraph:
    ...
    graph.add_node("store_conversation_summary", store_conversation_summary)
    ...
    graph.add_edge("finalize_response", "store_conversation_summary")
    graph.add_edge("store_conversation_summary", END)
...
class ChatService:
    ...
    def getResponse(self, message: str, chat_history: list[dict] | None = None):
        ...
        # Stream graph events node-by-node instead of blocking until completion
        for event in self.graph.stream(initial_state):
            for node_name, state_update in event.items():
                # Yield the final response from any terminal node, streamed word-by-word
                if node_name in ("finalize_response", "fallback_response", "confirm_store", "handle_neither"):
                    print("\n" + "=" * 60)
                    print("🏁 [END] Response ready, streaming to client...")
                    print("=" * 60 + "\n")
